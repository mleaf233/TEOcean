...
1. 和其他mod一样，将整个项目文件夹复制到 %AppData%/Balatro/mods 文件夹，先将本mod安装
1. 启动游戏。
2. 打开主菜单 → Mods。
3. 选择目标 mod（本 mod 即 The Eighth Ocean），进入 `配置` 标签。
4. 点击 `已适配的模组` 按钮，在每个模组框单独勾选每个模组即可。
5. 然后点击`返回`按钮或者点击 `手动重载` 按钮来触发。
